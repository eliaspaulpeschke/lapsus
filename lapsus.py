...
def main():
    ...
    parser = argparse.ArgumentParser(prog='lapsus',
                    description='adaptive timelapse utility for the raspberry pi',
                    epilog='BOTTOM TEXT')
    
    parser.add_argument("--base-interval", type=float, default=1.0, help=
            "base interval in which to capture an image and test if it is different than the last one, in seconds.")
    parser.add_argument("--max-interval", type=float, default = 0, help=
            "capture an image after this interval in seconds, even if it is no different than before. if set to zero or not set, only capture on change")
    parser.add_argument("--change", type=float, default=1000.0, help=
            "minimum change to justify saving an image. area of all changed areas big enough to count added together")
    ...
    while True:
        ...
        if (start_time - last_capture) > args.base_interval:
            ...
            if verbose:
                print(f"Captured. Difference: {norm}; Seconds since last save {last_capture - last_save}.")
...
def save_image(im):
    filename = f"{time.strftime('%d%m%y-%H%M%S')}-{math.modf(time.time())[0]}.jpg"
    if verbose:
